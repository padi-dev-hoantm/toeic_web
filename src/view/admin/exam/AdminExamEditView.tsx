...
const AdminExamEditView = () => {
  ...
  }; const { data, isFetchedAfterMount } = useQuerygetDetailByOwner(Number(examId))
  ...
  const handleSubmitOk = (value: any) => {
    const formatTime = formatDatePicker(value.exam_start_time)
    const newTime = `${formatTime?.replace(" ", "T")}.232Z`

    const dataSubmit = {
      id: value.id,
      exam_name: value.exam_name,
      exam_description: value.exam_description,
      exam_start_time: newTime,
        ...
        <form onSubmit={handleSubmit1(handleSubmitOk)} >
          <div className="box-shadow-item p-[20px] mt-[20px]">
            ...
            <DatePickerCommon
              ...
              control={control1}
              isRequired={true}
              defaultValue={dayjs(data?.data?.exam_start_time)}
              rules={{
                required: {
                  value: true,
                  message: "Đây là bắt buộc",
